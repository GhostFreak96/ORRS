	...
	<div class="wrap">
		...
		<h2><font color="blue">Passenger Detail</font></h2>
			...
			<table class="table">
				...
				<?php
				$san=$_SESSION['name'];
				$creditQuery="select credits from users where f_name='$san'";
				$somevariable=mysqli_query($conn,$creditQuery);			
				$row=mysqli_fetch_array($somevariable);
				$creditValue=$row['credits'];
				if($creditValue>2500){
					echo "<font color=brown><b>Credits Remaining:".$creditValue."</b></font>";
				}
				else{
					echo "<script>alert(\"Only $creditValue credits remaining.Minimum amount of credits is required to proceed.\")</script>";				
					echo "<script>
					setTimeout(function (){
